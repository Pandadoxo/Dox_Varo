    ...
    public void onMessageReceived(MessageReceivedEvent e) {
        ...
        if (args[0].startsWith(prefix) && e.isFromType(ChannelType.TEXT)) {
            ...
            if (!args[0].equalsIgnoreCase(""))
                if (!Bot.getCommandManager().sPerform(args[0], e.getMember(), e.getTextChannel(), e.getMessage())) {
        ...
        if (args[0].startsWith(prefix) && e.isFromType(ChannelType.PRIVATE)) {
            args[0] = args[0].substring(prefix.length());
            if (!args[0].equalsIgnoreCase(""))
                if (!Bot.getCommandManager().pPerform(args[0], e.getAuthor(), e.getPrivateChannel(), e.getMessage())) {
